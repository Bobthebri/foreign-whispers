...
def _synthesize_raw(
    tts_engine, text: str, wav_path: str, speaker_wav: str | None = None
) -> bytes | None:
...
def _synced_segment_audio(
    tts_engine, text: str, target_sec: float, work_dir,
    stretch_factor: float = 1.0, alignment_enabled: bool = True,
...
def text_file_to_speech(
    ...
    *alignment* overrides the module-level ``_ALIGNMENT_ENABLED`` flag.
    ...
    with tempfile.TemporaryDirectory() as synth_dir:
        def _do_synth(idx: int, text: str, seg_speaker: str | None) -> tuple[int, bytes | None]:
            wav_path = str(pathlib.Path(synth_dir) / f"seg_{idx}.wav")
            seg_wav = (
                voice_map.get(seg_speaker, speaker_wav)
